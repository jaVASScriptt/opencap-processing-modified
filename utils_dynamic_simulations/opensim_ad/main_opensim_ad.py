...
def run_tracking(baseDir, dataDir, subject, settings, case='0',
    ...
    if torque_driven_model:
        ...
        uw['CoordA'], lw['CoordA'], scaling['CoordA'] = bounds.getBoundsCoordinateDynamics(muscleDrivenJoints,
                                                                                           coordinate_optimal_forces)
    ...
    if withArms:
        # Arm activations.
        uw['ArmA'], lw['ArmA'], scaling['ArmA'] = bounds.getBoundsCoordinateDynamics(armJoints,
                                                                                     coordinate_optimal_forces)
    ...
    if withLumbarCoordinateActuators:
        # Lumbar activations.
        uw['LumbarA'], lw['LumbarA'], scaling['LumbarA'] = bounds.getBoundsCoordinateDynamics(lumbarJoints,
                                                                                              coordinate_optimal_forces)
    ...
    if torque_driven_model:
        ...
        uw['CoordE'], lw['CoordE'], scaling['CoordE'] = bounds.getBoundsCoordinateDynamics(muscleDrivenJoints,
                                                                                           coordinate_optimal_forces)
    ...
    if withArms:
        # Arm excitations.
        uw['ArmE'], lw['ArmE'], scaling['ArmE'] = bounds.getBoundsCoordinateDynamics(armJoints,
                                                                                     coordinate_optimal_forces)
        uw['ArmEk'] = ca.vec(uw['ArmE'].to_numpy().T * np.ones((1, N))).full()
        lw['ArmEk'] = ca.vec(lw['ArmE'].to_numpy().T * np.ones((1, N))).full()
    if withLumbarCoordinateActuators:
        # Lumbar excitations.
        uw['LumbarE'], lw['LumbarE'], scaling['LumbarE'] = bounds.getBoundsCoordinateDynamics(lumbarJoints,
                                                                                              coordinate_optimal_forces)
    ...
    if coordinate_constraints:
        ...
        for cons in coordinate_constraints:
            if cons == 'pelvis_ty':
                ...
                # bound.
                if not offset_ty:
                    ubQsk_vec[joints.index(cons), :-1] = coordinate_constraints[cons]['env_bound'] / \
                                                         scaling['Qs'].iloc[0][cons] + pelvis_ty_sc
                    lbQsk_vec[joints.index(cons), :-1] = -coordinate_constraints[cons]['env_bound'] / \
                                                         scaling['Qs'].iloc[0][cons] + pelvis_ty_sc
                ...
                ubQsk_vec[joints.index(cons), :-1] = coordinate_constraints[cons]['env_bound'] / scaling['Qs'].iloc[0][
                    cons] + c_sc
                lbQsk_vec[joints.index(cons), :-1] = -coordinate_constraints[cons]['env_bound'] / scaling['Qs'].iloc[0][
                    cons] + c_sc
    ...
    if solveProblem:
        ...
        if torque_driven_model:
            ...
            assert np.all(lw['CoordAk'] <= ca.vec(
                w0['CoordA'].to_numpy().T).full()), "Issue with lower bound coordinate activations"
            assert np.all(uw['CoordAk'] >= ca.vec(
                w0['CoordA'].to_numpy().T).full()), "Issue with upper bound coordinate activations"
            ...
            assert np.all(lw['CoordAj'] <= ca.vec(w0[
                                                      'CoordAj'].to_numpy().T).full()), "Issue with lower bound coordinate activations (collocation points)"
            assert np.all(uw['CoordAj'] >= ca.vec(w0[
                                                      'CoordAj'].to_numpy().T).full()), "Issue with upper bound coordinate activations (collocation points)"
        else:
            ...
            assert np.all(lw['Aj'] <= ca.vec(
                w0['Aj'].to_numpy().T).full()), "Issue with lower bound muscle activations (collocation points)"
            assert np.all(uw['Aj'] >= ca.vec(
                w0['Aj'].to_numpy().T).full()), "Issue with upper bound muscle activations (collocation points)"
            ...
            assert np.all(lw['Fj'] <= ca.vec(
                w0['Fj'].to_numpy().T).full()), "Issue with lower bound muscle forces (collocation points)"
            assert np.all(uw['Fj'] >= ca.vec(
                w0['Fj'].to_numpy().T).full()), "Issue with upper bound muscle forces (collocation points)"
        ...
        # Small margin to account for filtering.
        assert np.all(lw['Qsj'] - np.pi / 180 <= ca.vec(
            w0['Qsj'].to_numpy().T).full()), "Issue with lower bound coordinate values (collocation points)"
        assert np.all(uw['Qsj'] + np.pi / 180 >= ca.vec(
            w0['Qsj'].to_numpy().T).full()), "Issue with upper bound coordinate values (collocation points)"
        ...
        assert np.all(lw['Qdsj'] <= ca.vec(
            w0['Qdsj'].to_numpy().T).full()), "Issue with lower bound coordinate speeds (collocation points)"
        assert np.all(uw['Qdsj'] >= ca.vec(
            w0['Qdsj'].to_numpy().T).full()), "Issue with upper bound coordinate speeds (collocation points)"
        if withArms:
            ...
            assert np.all(
                lw['ArmAk'] <= ca.vec(w0['ArmA'].to_numpy().T).full()), "Issue with lower bound arm activations"
            assert np.all(
                uw['ArmAk'] >= ca.vec(w0['ArmA'].to_numpy().T).full()), "Issue with upper bound arm activations"
            ...
            assert np.all(lw['ArmAj'] <= ca.vec(
                w0['ArmAj'].to_numpy().T).full()), "Issue with lower bound arm activations (collocation points)"
            assert np.all(uw['ArmAj'] >= ca.vec(
                w0['ArmAj'].to_numpy().T).full()), "Issue with upper bound arm activations (collocation points)"
        if withLumbarCoordinateActuators:
            ...
            assert np.all(lw['LumbarAk'] <= ca.vec(
                w0['LumbarA'].to_numpy().T).full()), "Issue with lower bound lumbar activations"
            assert np.all(uw['LumbarAk'] >= ca.vec(
                w0['LumbarA'].to_numpy().T).full()), "Issue with upper bound lumbar activations"
            ...
            assert np.all(lw['LumbarAj'] <= ca.vec(
                w0['LumbarAj'].to_numpy().T).full()), "Issue with lower bound lumbar activations (collocation points)"
            assert np.all(uw['LumbarAj'] >= ca.vec(
                w0['LumbarAj'].to_numpy().T).full()), "Issue with upper bound lumbar activations (collocation points)"
        # Controls.
        if torque_driven_model:
            ...
            assert np.all(lw['CoordEk'] <= ca.vec(
                w0['CoordE'].to_numpy().T).full()), "Issue with lower bound coordinate excitations"
            assert np.all(uw['CoordEk'] >= ca.vec(
                w0['CoordE'].to_numpy().T).full()), "Issue with upper bound coordinate excitations"
        else:
            ...
            assert np.all(lw['ADtk'] <= ca.vec(
                w0['ADt'].to_numpy().T).full()), "Issue with lower bound muscle activation derivatives"
            assert np.all(uw['ADtk'] >= ca.vec(
                w0['ADt'].to_numpy().T).full()), "Issue with upper bound muscle activation derivatives"
        if withArms:
            ...
            assert np.all(
                lw['ArmEk'] <= ca.vec(w0['ArmE'].to_numpy().T).full()), "Issue with lower bound arm excitations"
            assert np.all(
                uw['ArmEk'] >= ca.vec(w0['ArmE'].to_numpy().T).full()), "Issue with upper bound arm excitations"
        if withLumbarCoordinateActuators:
            ...
            assert np.all(lw['LumbarEk'] <= ca.vec(
                w0['LumbarE'].to_numpy().T).full()), "Issue with lower bound lumbar excitations"
            assert np.all(uw['LumbarEk'] >= ca.vec(
                w0['LumbarE'].to_numpy().T).full()), "Issue with upper bound lumbar excitations"
        if not torque_driven_model:
            ...
            assert np.all(
                lw['FDtk'] <= ca.vec(w0['FDt'].to_numpy().T).full()), "Issue with lower bound muscle force derivatives"
            assert np.all(
                uw['FDtk'] >= ca.vec(w0['FDt'].to_numpy().T).full()), "Issue with upper bound muscle force derivatives"
        ...
        assert np.all(lw['Qddsk'] <= ca.vec(
            w0['Qdds'].to_numpy().T).full()), "Issue with lower bound coordinate speed derivatives"
        assert np.all(uw['Qddsk'] >= ca.vec(
            w0['Qdds'].to_numpy().T).full()), "Issue with upper bound coordinate speed derivatives"
        ...
        if withReserveActuators:
            ...
            for c_j in reserveActuatorCoordinates:
                ...
                assert np.all(lw['rActk'][c_j] <= ca.vec(
                    w0['rAct'][c_j].to_numpy().T).full()), "Issue with lower bound reserve actuators"
                assert np.all(uw['rActk'][c_j] >= ca.vec(
                    w0['rAct'][c_j].to_numpy().T).full()), "Issue with upper bound reserve actuators"
    ...
    if analyzeResults:
        ...
        if writeGUI:
            ...
            from tools.utils import numpy_to_storage
            labels = ['time'] + joints
            if torque_driven_model:
        ...
        for k in range(N):
            ...
            for j in range(d):
                if torque_driven_model:
                    ...
                    coordExcitationTerm_opt_all += weights['coordinateExcitationTerm'] * coordExcitationTerm_opt * h * \
                                                   B[j + 1]
                else:
                ...
                positionTrackingTerm_opt = f_NQsToTrackWSum2(Qskj_opt[idx_coordinates_toTrack, 0],
                                                             dataToTrack_Qs_sc_offset_opt[:, k], w_dataToTrack)
                velocityTrackingTerm_opt = f_NQsToTrackWSum2(Qdskj_opt[idx_coordinates_toTrack, 0],
                                                             dataToTrack_Qds_sc[:, k], w_dataToTrack)
                positionTrackingTerm_opt_all += weights['positionTrackingTerm'] * positionTrackingTerm_opt * h * B[
                    j + 1]
                velocityTrackingTerm_opt_all += weights['velocityTrackingTerm'] * velocityTrackingTerm_opt * h * B[
                    j + 1]
                jointAccelerationTerm_opt_all += weights['jointAccelerationTerm'] * jointAccelerationTerm_opt * h * B[
                    j + 1]
                if withArms:
                ...
                if withLumbarCoordinateActuators:
                    ...
                    lumbarExcitationTerm_opt_all += weights['lumbarExcitationTerm'] * lumbarExcitationTerm_opt * h * B[
                        j + 1]
                if trackQdds:
