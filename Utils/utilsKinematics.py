...
class Kinematics:
    ...
    def rotate_com(self, comValues, euler_angles):
        ...
        rotated_com = pd.DataFrame(
            data=np.concatenate((np.expand_dims(comValues['time'].to_numpy(), axis=1), rotated_com), axis=1),
            columns=['time', 'x', 'y', 'z'])
    ...
    def get_coordinate_values(self, in_degrees=True,
        ...
        if lowpass_cutoff_frequency > 0:
            ...
            if self.lowpass_cutoff_frequency_for_coordinate_values > 0:
                print(
                    "Warning: You are filtering the coordinate values a second time; coordinate values were filtered when creating your class object.")
