        ...
        if thisTime <= idTime[-1]:
            ...
            for iCoord, coord in enumerate(coords):
                if '_beta' not in coord.getName():
                    ...
                    thisController = opensim.PrescribedController.safeDownCast(
                        controllerSet.get(coord.getName() + '_controller'))
            ...
            if thisTime <= endTime:
                    ...
                    if '_beta' not in coord.getName():
                        ...
                        if usingGenForceActuators:
                            ...
                            if len(actuatorName) == 1:
                                ...
                                thisController = opensim.PrescribedController.safeDownCast(
                                    controllerSet.get(coord.getName() + '_controller'))
                                thisConstFxn = opensim.Constant.safeDownCast(
                                    thisController.get_ControlFunctions(0).get(0))
                ...
                for iMusc, muscName in enumerate(muscleNames):
                    if muscForceOverride:
                        ...
                        muscles.get(iMusc).setOverrideActuation(state, overrideForceTable.getDependentColumn(muscName)[
                            overrideForceRow])
                    else:
